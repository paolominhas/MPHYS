...
def load_experimental(filepath):
    ...
    n_accepted = n_total - n_few - n_chi2 - n_short
    # Cluster factor: average centroids per accepted track
    # Centroids within a track are correlated (shared δx, adjacent-pad
    # charge sharing, same selection), so the effective independent sample
    # size is N_tracks, not N_centroids.  Errors must be inflated by
    # sqrt(cluster_factor).  [ALICE JINST 5 (2010) P09002, Sec. 6.1]
    cluster_factor = len(arr) / max(n_accepted, 1)
    print(f"    {n_total} tracks → {n_accepted} accepted  "
          f"→ {len(arr)} centroid values  "
          f"(cluster factor = {cluster_factor:.1f})")
    ...
    return arr[arr < np.percentile(arr, 95)], cluster_factor
...
def load_simulation(filepath, low_mev=0.0):
    ...
    print(f"    {len(vals):,} events  ({vals.min():.2f}–{vals.max():.1f} MeV)"
          + (f"  [bump below {low_mev:.0f} MeV shown but excluded from fit]"
             if low_mev > 0 else ""))
    return vals, wts
...
def fit_on_edges(values, weights, edges, peak, label="", cluster_factor=1.0):
    """Fit Moyal to bottom TRUNCATION fraction on shared edges.

    cluster_factor: inflate Poisson errors by sqrt(cluster_factor) to
    account for intra-track correlations (centroid entries from the same
    track are not independent).  For simulation (one entry per event),
    cluster_factor = 1.  For experimental data, cluster_factor ≈ average
    centroids per track (~3–8).
    """
    ...
    yn = counts / peak
    # Effective Poisson error inflated by sqrt(cluster_factor) to account
    # for within-track correlations [cf. ALICE JINST 5 (2010) P09002 Sec 6]
    ye = np.maximum(np.sqrt(counts * cluster_factor), 1.0) / peak
...
# PLOT
# ══════════════════════════════════════════════════════════════════════════════
def make_comparison_plot(pair, exp_vals, sim_vals, sim_wts, cluster_factor=1.0):
    low_mev  = pair["sim_low_mev"]
    is_muon  = low_mev > 0

    # ── Peak of each distribution (coarse histogram mode) ─────────────────────
    def coarse_peak(v, w=None, lo_pct=20):
        """Find histogram mode above the lo_pct percentile.
        Skips threshold/noise contamination at very low dE/dx."""
        lo_cut = float(np.percentile(v, lo_pct))
        mask   = v > lo_cut
        vm     = v[mask]
        wm     = w[mask] if w is not None else None
        c, e   = np.histogram(vm, bins=200, weights=wm)
        return float(0.5*(e[:-1]+e[1:])[np.argmax(c)])

    exp_pk_raw = coarse_peak(exp_vals)
    # For muon sim: find peak ABOVE the low-E bump so normalisation is correct
    if is_muon and low_mev > 0:
        above = sim_vals > low_mev
        sim_pk_raw = coarse_peak(sim_vals[above], sim_wts[above])
    else:
    ...
    exp_pk_n   = float(exp_cnt.max())
    # For muon sim: normalise peak from bins ABOVE bump only
    if is_muon and low_mev > 0:
        bump_edge_temp = low_mev / sim_pk_raw
        non_bump = bc > bump_edge_temp
        sim_pk_n = float(sim_cnt[non_bump].max()) if non_bump.sum() > 0 else float(sim_cnt.max())
    else:
        sim_pk_n = float(sim_cnt.max())
    exp_yn = exp_cnt / exp_pk_n
    # Inflate data errors by sqrt(cluster_factor): centroids within a
    # track are correlated, so effective N_indep ~ N_centroids / n_per_track
    exp_ye = np.maximum(np.sqrt(exp_cnt * cluster_factor), 1.) / exp_pk_n
    sim_yn = sim_cnt / sim_pk_n
    sim_ye = np.maximum(np.sqrt(sim_cnt), 1.) / sim_pk_n
    ...
    # Fit simulation excluding bump region
    if is_muon and low_mev > 0:
        above_bump = sim_norm > bump_edge
        print("  Fitting simulation (excluding bump)...");  r_s = fit_on_edges(
            sim_norm[above_bump], sim_wts[above_bump], edges, sim_pk_n, "Simulation")
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
# OVERLAY PLOT — two runs on the same figure, one simulation
# ══════════════════════════════════════════════════════════════════════════════
C_RUN1 = "#1f77b4"   # blue   — run 0006
C_RUN2 = "#ff7f0e"   # orange — run 0042

def make_overlay(title, output, run1, run2, sim_vals, sim_wts,
                 low_mev=0.0):
    """
    run1, run2: dicts with keys  exp_vals, r_d, r_s, epk
    sim_vals/sim_wts: shared simulation (already loaded)
    """
    from matplotlib.lines  import Line2D
    from matplotlib.patches import Patch
    ...
    if is_muon and low_mev > 0:
        above = sim_vals > low_mev
        sim_pk = coarse_peak(sim_vals[above], sim_wts[above])
    else:
        sim_pk = coarse_peak(sim_vals, sim_wts)
    sim_norm  = sim_vals / sim_pk
    bump_edge = low_mev / sim_pk if is_muon else 0.0

    # ── Normalise each run independently ──────────────────────────────────────
    r1_norm = run1["exp_vals"] / run1["epk"]
    r2_norm = run2["exp_vals"] / run2["epk"]

    # ── Shared edges ──────────────────────────────────────────────────────────
    x_max = min(np.percentile(r1_norm, 99.5),
                np.percentile(r2_norm, 99.5),
                np.percentile(sim_norm, 99.5), 8.0)
    edges = np.linspace(0, x_max, N_BINS + 1)
    bc    = 0.5*(edges[:-1]+edges[1:]);  bw = float(edges[1]-edges[0])
    ...
    bm = bc <= bump_edge
    if is_muon and bm.sum() > 0:
        sim_pk_n = float(sim_cnt[~bm].max()) if (~bm).sum() > 0 \
                   else float(sim_cnt.max())
    else:
        sim_pk_n = float(sim_cnt.max())
    sim_yn = sim_cnt / sim_pk_n

    r1_cnt, _ = np.histogram(r1_norm, bins=edges)
    r2_cnt, _ = np.histogram(r2_norm, bins=edges)
    r1_pk = float(r1_cnt.max()); r2_pk = float(r2_cnt.max())
    cf1 = run1.get("cf", 1.0)
    cf2 = run2.get("cf", 1.0)
    r1_yn = r1_cnt / r1_pk
    r1_ye = np.maximum(np.sqrt(r1_cnt * cf1), 1.) / r1_pk
    r2_yn = r2_cnt / r2_pk
    r2_ye = np.maximum(np.sqrt(r2_cnt * cf2), 1.) / r2_pk

    # ── Sim fit (from first run — same sim file, same fit) ────────────────────
    r_s   = run1["r_s"]
    cut_s = r_s["cut"] if r_s else x_max * TRUNCATION
    ...
    # ── Simulation histogram (background) ─────────────────────────────────────
    s_fit_m = (~bm) & (bc <= cut_s);  s_tail = (~bm) & (~s_fit_m)
    ax.bar(bc[s_fit_m], sim_yn[s_fit_m], width=bw*0.88,
           color=C_SIM, alpha=0.30, linewidth=0, zorder=2)
    ax.bar(bc[s_tail], sim_yn[s_tail], width=bw*0.88,
           color=C_SIM, alpha=0.10, hatch="////",
           edgecolor=C_SIM, linewidth=0.5, zorder=2)
    if is_muon and bm.sum() > 0:
        ax.bar(bc[bm], sim_yn[bm], width=bw*0.88,
               color=C_SIM, alpha=0.35, edgecolor=C_SIM,
    ...
    # ── Data: two runs as error bars ──────────────────────────────────────────
    ax.errorbar(bc, r1_yn, yerr=r1_ye, fmt="o", color=C_RUN1,
                ms=5, lw=1.2, capsize=2.5, zorder=6)
    ax.errorbar(bc, r2_yn, yerr=r2_ye, fmt="D", color=C_RUN2,
                ms=4.5, lw=1.2, capsize=2.5, zorder=6)

    # ── Fit curves ────────────────────────────────────────────────────────────
    xs = np.linspace(0, x_max, 3000)
    if run1["r_d"]:
        ax.plot(xs, _moyal(xs, *run1["r_d"]["popt"]),
                color=C_RUN1, lw=2.0, ls="-", zorder=8)
    if run2["r_d"]:
        ax.plot(xs, _moyal(xs, *run2["r_d"]["popt"]),
                color=C_RUN2, lw=2.0, ls="-", zorder=8)
    if r_s:
        xs_fit = np.linspace(0, cut_s, 2000)
        ax.plot(xs_fit, _moyal(xs_fit, *r_s["popt"]),
                color=C_SIM, lw=2.2, ls="--", zorder=7)
    ...
    # Truncation line
    ax.axvline(cut_s, color="dimgray", lw=1.0, ls=":", alpha=0.75, zorder=3)

    # ── Legend ────────────────────────────────────────────────────────────────
    leg1_handles = [
        Line2D([0],[0], color=C_RUN1, lw=2.0, ls="-", marker="o",
               ms=4, label="Run 0006  — data + fit"),
        Line2D([0],[0], color=C_RUN2, lw=2.0, ls="-", marker="D",
               ms=3.5, label="Run 0042  — data + fit"),
        Line2D([0],[0], color=C_SIM,  lw=2.2, ls="--",
               label="Simulation  — Landau fit"),
        Patch(facecolor=C_SIM, alpha=0.30, label="Sim. histogram"),
    ]
    leg1 = ax.legend(handles=leg1_handles, loc="upper right",
                     framealpha=0.92, edgecolor="#aaaaaa",
                     handlelength=1.6, labelspacing=0.28,
    ...
    def fmt_r(label, r):
        if r is None: return f"{label}: fit failed"
        return (f"{label}:  MPV={r['mpv']:.3f},"
                f"  $\\xi$={r['xi']:.3f},"
                f"  $\\chi^2/\\nu$={r['chi2_red']:.2f}")
    leg2_handles = [
        Line2D([0],[0], lw=0, label=fmt_r("Run 06", run1["r_d"])),
        Line2D([0],[0], lw=0, label=fmt_r("Run 42", run2["r_d"])),
        Line2D([0],[0], lw=0, label=fmt_r("Sim.",   r_s)),
    ]
    leg2 = ax.legend(handles=leg2_handles, loc="upper center",
    ...
    # ── Axes ──────────────────────────────────────────────────────────────────
    ax.set_yscale("log")
    pos = np.concatenate([r1_yn[r1_yn>0], r2_yn[r2_yn>0], sim_yn[sim_yn>0]])
    ax.set_ylim(pos.min()*0.3, 3.5)
    ax.set_xlim(0, x_max)
    ax.yaxis.set_major_locator(ticker.LogLocator(base=10,subs=(1.,),numticks=5))
    ax.yaxis.set_minor_locator(
    ...
    # ── Pull panel (entire x range) ─────────────────────────────────────────
    for vals_yn, vals_ye, r_fit, color, mk in [
            (r1_yn, r1_ye, run1["r_d"], C_RUN1, "o"),
            (r2_yn, r2_ye, run2["r_d"], C_RUN2, "D")]:
        if r_fit is None: continue
        yf  = _moyal(bc, *r_fit["popt"])
        den = np.sqrt(np.maximum(np.abs(yf), 1e-10))
        pls = np.clip((vals_yn - yf)/den, -8, 8)
        perr = np.clip(vals_ye / den, 0, 8)
        fm  = r_fit["fm"]
        # Fitted region — full opacity
        axr.errorbar(bc[fm], pls[fm], yerr=perr[fm],
                     fmt=mk, color=color, ms=4, lw=1.0,
                     capsize=2, zorder=4)
        # Tail region — fainter
        tail = ~fm & (vals_yn > 0)
        if tail.sum() > 0:
            axr.errorbar(bc[tail], pls[tail], yerr=perr[tail],
                         fmt=mk, color=color, alpha=0.3, ms=3,
                         lw=0.7, capsize=1.5, zorder=3)

    axr.axhline(0,  color="black",   lw=0.9)
    axr.axhline(+2, color="#888888", lw=0.9, ls="--", label=r"$\pm2\sigma$")
    axr.axhline(-2, color="#888888", lw=0.9, ls="--")
    axr.axvline(cut_s, color="dimgray", lw=1., ls=":", alpha=0.7)
    axr.set_ylim(-6.5, 6.5)
    axr.yaxis.set_major_locator(ticker.MultipleLocator(2.))
    axr.legend(loc="upper right", fontsize=8, framealpha=0.92,
               edgecolor="#aaaaaa", handlelength=1.3, borderpad=0.4)
    axr.set_ylabel(r"$(N-F)/\!\sqrt{F}$", fontsize=10)
    ...
    for ext in [".pdf", ".png"]:
        fig.savefig(f"{OUTDIR}/{output}{ext}",
                    dpi=300 if ext==".png" else None, bbox_inches="tight")
...
# ══════════════════════════════════════════════════════════════════════════════
# REDUCED LANDAU VARIABLE PLOT — tails aligned
# λ = (x/MPV_peak - MPV_fit) / ξ_fit   [Landau (1944); PDG Sec. 34]
# In these coordinates the Moyal PDF is universal: if the underlying
# energy loss is Landau-like, both distributions collapse onto one curve.
# ══════════════════════════════════════════════════════════════════════════════
def make_reduced_plot(pair, exp_norm, sim_norm, sim_wts,
                      r_d, r_s, cluster_factor=1.0):
    """
    Plot data and simulation on the reduced Landau axis
    λ = (x - MPV) / ξ, using each distribution's own fitted MPV and ξ.

    References:
      Landau L.D., J. Phys. USSR 8 (1944) 201
      Moyal J.E., Phil. Mag. 46 (1955) 263
      PDG, Phys. Rev. D 98 (2018) 030001, Sec. 34.2
      Bichsel H., Rev. Mod. Phys. 60 (1988) 663, Sec. III
    """
    from matplotlib.lines import Line2D

    if r_d is None or r_s is None:
        print("  SKIP reduced plot — fit missing"); return

    # ── Transform to reduced variable ─────────────────────────────────────
    lam_d = (exp_norm - r_d["mpv"]) / r_d["xi"]
    lam_s = (sim_norm - r_s["mpv"]) / r_s["xi"]

    # ── Shared edges in λ space ───────────────────────────────────────────
    lam_lo = max(float(np.percentile(lam_d, 0.5)),
                 float(np.percentile(lam_s, 0.5)), -5.0)
    lam_hi = min(float(np.percentile(lam_d, 99.5)),
                 float(np.percentile(lam_s, 99.5)), 25.0)
    n_bins = 50
    edges  = np.linspace(lam_lo, lam_hi, n_bins + 1)
    bc     = 0.5 * (edges[:-1] + edges[1:])
    bw     = float(edges[1] - edges[0])

    # ── Normalised histograms (unit area) ─────────────────────────────────
    d_cnt, _ = np.histogram(lam_d, bins=edges)
    s_cnt, _ = np.histogram(lam_s, bins=edges, weights=sim_wts)

    d_norm_f = max(float(d_cnt.sum()) * bw, 1.0)
    s_norm_f = max(float(s_cnt.sum()) * bw, 1.0)

    d_yn = d_cnt / d_norm_f
    d_ye = np.maximum(np.sqrt(d_cnt * cluster_factor), 1.) / d_norm_f
    s_yn = s_cnt / s_norm_f
    s_ye = np.maximum(np.sqrt(s_cnt), 1.) / s_norm_f

    # ── Universal Moyal curve in λ coordinates ────────────────────────────
    # Moyal PDF with loc=0, scale=1 is the universal reduced form
    lam_smooth = np.linspace(lam_lo, lam_hi, 2000)
    moyal_univ = sc.moyal.pdf(lam_smooth, loc=0, scale=1)

    # Scale to match histogram normalisation (unit area, so just PDF × 1)
    # No scaling needed if histograms are true densities

    # ── Figure ────────────────────────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(8, 5.5))

    # Data
    ax.errorbar(bc, d_yn, yerr=d_ye, fmt="o", color=C_DATA,
                ms=5, lw=1.2, capsize=2.5, label="Data", zorder=5)
    # Simulation
    ax.errorbar(bc, s_yn, yerr=s_ye, fmt="s", color=C_SIM,
                ms=4.5, lw=1.0, capsize=2, label="Simulation", zorder=4)
    # Universal Moyal
    ax.plot(lam_smooth, moyal_univ, color="black", lw=2.0, ls="--",
            label="Universal Moyal", zorder=6)

    ax.set_yscale("log")
    pos = np.concatenate([d_yn[d_yn > 0], s_yn[s_yn > 0]])
    ax.set_ylim(pos.min() * 0.3, pos.max() * 3)
    ax.set_xlim(lam_lo, lam_hi)

    ax.set_xlabel(r"Reduced Landau variable  $\lambda = (x - \mathrm{MPV}) / \xi$",
                  fontsize=13)
    ax.set_ylabel("Probability density", fontsize=13)
    ax.set_title(f"{pair['title']}  —  reduced Landau variable", fontsize=12, pad=6)

    # Annotation
    ax.text(0.97, 0.97,
            f"Data:   MPV={r_d['mpv']:.3f},  $\\xi$={r_d['xi']:.3f}\n"
            f"Sim:    MPV={r_s['mpv']:.3f},  $\\xi$={r_s['xi']:.3f}\n"
            f"$\\xi$ ratio = {r_d['xi']/r_s['xi']:.2f}",
            transform=ax.transAxes, va="top", ha="right",
            fontsize=9, family="monospace",
            bbox=dict(boxstyle="round,pad=0.4", facecolor="white",
                      edgecolor="#aaaaaa", alpha=0.92))

    leg = ax.legend(loc="upper left", fontsize=10, framealpha=0.92,
                    edgecolor="#aaaaaa")

    try:
        lo = hep.cms.label(loc=2, data=True, label="Preliminary",
                           rlabel="HIBEAM", ax=ax)
        lo[0].set_text("ESS")
    except Exception: pass

    plt.tight_layout()
    out_base = pair["output"].replace("comparison_", "reduced_")
    for ext in [".pdf", ".png"]:
        fig.savefig(f"{OUTDIR}/{out_base}{ext}",
...
for idx, pair in enumerate(PAIRS):
    print(f"\n{'='*58}\n  {pair['title']}\n{'='*58}")
    try:
        exp_vals, cf = load_experimental(pair["exp_file"])
    ...
    # ── Reduced Landau variable plot (tail alignment) ─────────────────────
    exp_norm = exp_vals / epk
    sim_norm = sim_vals / spk
    print("  Reduced Landau plot...")
    make_reduced_plot(pair, exp_norm, sim_norm, sim_wts, r_d, r_s,
                      cluster_factor=cf)

    raw_data[idx] = {"exp_vals": exp_vals, "sim_vals": sim_vals,
                     "sim_wts": sim_wts, "r_d": r_d, "r_s": r_s,
                     "epk": epk, "spk": spk, "cf": cf}

# ── Overlay plots: two runs per physics channel ──────────────────────────────
if 0 in raw_data and 1 in raw_data:
    print(f"\n{'='*58}\n  Krakow overlay\n{'='*58}")
    make_overlay(
        title    = r"Krakow $^2$H(p,p)  —  run 0006 + 0042 overlay",
        output   = "overlay_krakow",
        run1     = raw_data[0],  run2 = raw_data[1],
        sim_vals = raw_data[0]["sim_vals"],
        sim_wts  = raw_data[0]["sim_wts"],
        low_mev  = 0.0)

if 2 in raw_data and 3 in raw_data:
    print(f"\n{'='*58}\n  Muon overlay\n{'='*58}")
    make_overlay(
        title    = "Muon lab (MCPL)  —  run 0006 + 0042 overlay",
        output   = "overlay_muon",
        run1     = raw_data[2],  run2 = raw_data[3],
        sim_vals = raw_data[2]["sim_vals"],
        sim_wts  = raw_data[2]["sim_wts"],
        low_mev  = 15.0)
...
with open(f"{OUTDIR}/fit_results_all_runs.txt", "w") as fout:
    ...
    fout.write("Error model: Poisson errors inflated by sqrt(cluster_factor)\n")
    fout.write("  to account for intra-track centroid correlations\n\n")
