...
def fit_landau(values, truncation=0.70, n_bins=40):
    """
    Fit Moyal to the lower `truncation` fraction, normalised to peak = 1.

    This matches the fit_on_edges() approach in dedx_data_sim_comparison.py:
      - Histogram normalised by its own peak bin count before fitting
      - Errors:  sqrt(counts) / peak   (Poisson / peak, no minimum-count gate)
      - Truncation cut from np.percentile(values, truncation*100) on raw values
      - Bounds: [bc[0], 1e-9, 0] → [cut, cut, norm0*20]
      - Initial norm = peak * bin_width

    The returned dict stores both normalised fit parameters (popt) used for
    chi² and xi reporting, and a raw-scale curve (fit_curve, popt_raw) that
    _draw_panels uses to overlay the fit on the raw-count histogram.

    References
    ----------
    [1] Landau, L.D. (1944). J. Phys. USSR 8, 201.
    [2] Moyal, J.E. (1955). Phil. Mag. 46, 263.
    [3] ALICE Collaboration (2010). JINST 5, P09002.  [truncated-mean method]
    """
    if len(values) < 50:
        raise ValueError(f"Only {len(values)} events — need >= 50.")

    # ── Build histogram over full display range ───────────────────────────
    ...
    # ── Truncation cut on raw values ─────────────────────────────────────
    cut_value = float(np.percentile(values, truncation * 100.0))
    fit_mask  = bin_centers <= cut_value

    if fit_mask.sum() < 4:
        raise RuntimeError(
            f"Only {fit_mask.sum()} bins in fit region — try reducing n_bins.")

    # ── Peak normalisation (same as fit_on_edges) ─────────────────────────
    peak = float(counts.max())
    yn   = counts / peak
    # Poisson error divided by peak — no minimum-count gate applied here;
    # the max(…, 1) floor only affects the error of empty bins (Garwood bound)
    ye   = np.maximum(np.sqrt(counts), 1.0) / peak

    xf, yf, ef = bin_centers[fit_mask], yn[fit_mask], ye[fit_mask]

    # ── Initial parameters ────────────────────────────────────────────────
    loc0   = float(xf[np.argmax(yf)])
    scale0 = max((cut_value - float(bin_centers[0])) / 8.0, 1e-9)
    norm0  = peak * bin_width          # ≈ integral of normalised hist in [0, cut]
    ...
    try:
        popt, pcov = optimize.curve_fit(
            ...
            bounds=([float(bin_centers[0]), 1e-9, 0.0],
                    [cut_value, cut_value, norm0 * 20.0]),
            maxfev=40_000)
    except RuntimeError as exc:
        raise RuntimeError(f"Fit did not converge: {exc}") from exc

    loc, scale, norm_param = popt
    loc_err, scale_err, norm_err = np.sqrt(np.diag(pcov))

    # ── Goodness of fit (normalised space, fitted bins only) ───────────────
    y_pred   = _moyal_scaled(xf, *popt)
    chi2     = float(np.sum(((yf - y_pred) / ef) ** 2))
    ndf      = int(fit_mask.sum()) - 3
    chi2_red = chi2 / max(ndf, 1)
    p_value  = float(stats.chi2.sf(chi2, max(ndf, 1)))

    # ── Fit curves ────────────────────────────────────────────────────────
    # Normalised (peak = 1) — used for chi² checks
    fit_curve_norm = _moyal_scaled(bin_centers, *popt)
    # Raw count scale — used by _draw_panels for visual overlay
    # norm in raw space = norm_param * peak (since yn = counts/peak)
    popt_raw   = [loc, scale, norm_param * peak]
    fit_curve  = _moyal_scaled(bin_centers, *popt_raw)

    # ── Tail chi² (raw counts, Pearson) ───────────────────────────────────
    ...
    return {
        # Landau parameters (normalised space)
        "loc": loc,       "loc_err": loc_err,
        "scale": scale,   "scale_err": scale_err,
        "norm": norm_param, "norm_err": norm_err,
        "mpv": loc,
        # Raw parameters for plotting
        "popt": popt,          # normalised — for chi², xi reporting
        "popt_raw": popt_raw,  # raw counts  — passed to _draw_panels curve
        "pcov": pcov,
        # Goodness of fit
        "chi2": chi2,          "ndf": ndf,
        ...
        "bin_width":   bin_width,
        "counts":      counts,          # raw, for bar display
        "fit_curve":   fit_curve,       # raw, for pull denominator
        "yn":          yn,              # normalised, for reference
        "ye":          ye,
        "fit_curve_norm": fit_curve_norm,
        "peak":        peak,
        "fit_mask":    fit_mask,
...
def _draw_panels(ax_top, ax_res, fit_result, x_label, x_max_display, log_y):
    ...
    fc  = fit_result["fit_curve"]          # raw-scale curve for pulls
    fm  = fit_result["fit_mask"]
    bw  = fit_result["bin_width"]
    # Use popt_raw (raw count scale) for the smooth overlay line
    popt_plot = fit_result.get("popt_raw", fit_result["popt"])
    ...
    # ── Fit curve (raw-count scale) ───────────────────────────────────────
    x_dense = np.linspace(float(bc[0]), float(x_max_display), 2000)
    y_dense = _moyal_scaled(x_dense, *popt_plot)
